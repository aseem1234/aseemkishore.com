...
- API routes: `POST /api/tools/tweet-score` (Together writer, 20 req/h/IP), `POST /api/tools/share-card` (OpenAI GPT Image 2.5 Flare → GPT Image 2 background + `sharp` overlay, 10 req/h/IP, dark-gradient fallback without `OPENAI_API_KEY`), `GET /api/gateway-canary` (Vercel Cron `17 11 * * *` UTC; inert unless armed). Rate limits are `src/lib/rate-limit.ts`, in-memory per warm instance — cost control, not security.
